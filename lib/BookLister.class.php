...
class BookLister {
	...
	function fetch_book_list_by_query() {
		...
		RIGHT JOIN categories c ON (bc.book_id = b.id OR b.category=c.id)
		...
		*/
		// GROUP_CONCAT(DISTINCT c.id SEPARATOR ',')
		$baseSQL = "SELECT DISTINCT b.*, i.filename
		FROM books b
		LEFT JOIN uploaded_images i ON b.image_id = i.id
		INNER JOIN books_categories bc ON (bc.book_id = b.id)
		INNER JOIN categories c ON bc.cat_id = c.id
		WHERE b.visibility=:visibility";
		{
		...
		//echo file_put_contents(SITE_PATH."/test.txt", $baseSQL);
	...
	function fetch_book_list_by_account($uploaderID, $page) {
		...
		// Apply constant bindings
		$stmt->bindValue("uploader_id", $uploaderID, PDO::PARAM_INT );
		$stmt->bindValue("page", ($page-1)*10, PDO::PARAM_INT );
		// Execute the statement
		$stmt->execute();
