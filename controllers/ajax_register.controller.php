...
$methods['run'] = function($instance) {
	try {
		...
		$thisHereLocation = "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];
		...
		// Send new account notice
		{
			$tmpl = new Template();
			$tmpl->set_template_file(SITE_PATH.'/templates/email_new_account.template.php');
			$tmpl->activation_link = $activLink;
			$tmpl->ss = $ss;
			$tmpl->email = htmlentities($_POST['email']);
			$tmpl->name  = htmlentities($_POST['name']);
