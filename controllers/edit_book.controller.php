...
$methods['run'] = function($instance) {
	...
	if ($goodToGo) {
		...
		if ($bookinfo['exists'] === true) {
			...
			if (!$userOwnsBook && ! ($_SESSION['is_admin_user'] === "yes")) {
				die('Can not edit: not book owner!');
			}

			if ($_POST) {
				$sitedb->update_book_from_sanitized_data($idToGet,
					$_POST['isbn'],
					$_POST['book_title'],
					$_POST['book_author'],
				...
				header('Location: '.WEB_PATH.'?location=book/'.$idToGet);
			}

			// Setup the template
			$bookview_tmpl = new Template();
			$bookview_tmpl->set_template_file(SITE_PATH.'/templates/edit_book.template.php');
			...
			$bookview_tmpl->categories = $sitedb->list_categories();
